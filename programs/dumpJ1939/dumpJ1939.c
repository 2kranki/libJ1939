...
)
{
    ...
    pStrTime = objArray_Get(pRecord, 1);
    BREAK_NULL(pStrTime);
    pTime = AStr_getData(pStrTime);
    BREAK_NULL(pTime);
    
    pStrMsgID = objArray_Get(pRecord, 2);
    BREAK_NULL(pStrMsgID);
    pMsgID = AStr_getData(pStrMsgID);
    BREAK_NULL(pMsgID);
    
    pStrLength = objArray_Get(pRecord, 3);
    BREAK_NULL(pStrLength);
    pLength = AStr_getData(pStrLength);
    BREAK_NULL(pLength);
    
    pStrData = objArray_Get(pRecord, 4);
    BREAK_NULL(pStrData);
    pData = AStr_getData(pStrData);
    BREAK_NULL(pData);

    fRc = hex_ScanUint32A(AStr_getLength(pStrTime), pTime, &time);
    BREAK_FALSE(fRc);
    //msg.CMSGSID.CMSGTS = time;
    ...
    fRc = j1939msg_ConstructMsg_E1( &msg, msgID, length, bytes);
    BREAK_FALSE(fRc);
    msg.CMSGSID.CMSGTS = time;
...
)
{
    ...
    if (pRecord) {
        ...
        fprintf(stderr, "FATAL ERROR - NULL Input record array!");
        exit(99);
    }
    
    fRc = constructMessage(pVoid, pRecord);
       ...
       pEntry = objArray_Get(pRecords, i+1);
        if (pEntry) {
            if (obj_getIdent(pEntry) == OBJ_IDENT_OBJARRAY) {
                ...
                if (jMax >= 3) {
                    fRc = constructMessage(NULL, pEntry);
                    j1939misc_DumpMessage(pMisc, &msg);
                    ++numMsgs;
                }
            }
...
)
{
    ...
    if (pFilePath) {
        ...
        if (pPath) {
            ...
            dbCsv_setRecordProcess(pCsv, printMessage, NULL);
        }
    }
    else {
        fprintf(stderr, "FATAL ERROR - Missing input file path!");
...
)
{
    ...
    pMisc = j1939misc_New( );
    BREAK_NULL(pMisc);
    j1939misc_setFileOut(pMisc, stdout);
